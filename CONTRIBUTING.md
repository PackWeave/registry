...
> [!TIP]
> Test your pack before opening a PR: `weave install ./src/your-pack-name` — idempotent and re-reads files on each run.

> [!IMPORTANT]
> Never edit `packs/` or `index.json` directly — they are auto-generated by CI on every merge to main. Only ever touch files under `src/`.
...
> [!WARNING]
> Never store actual secret values in `pack.toml`. Use `[servers.env.KEY]` declarations only — the real values come from the user's shell environment at runtime.
